...
Now let’s apply these questions to a flexi-cap fund using [Deepdive](https://deepdive.fundinvestigator.com/). The goal is not to label the fund “good” or “bad.” Instead, we will show what each question reveals and how you can use the same framework to assess any fund.
...
| Question | HDFC Flexi Cap vs Nifty 500 TRI | What does it show? |
|---|---|---|
| Did the fund beat a fair benchmark? | CAGR: 19.7% vs 15.5%<br>IRR: 19.9% vs 14.2%[^hdfc-sip] | Both the lump-sum investment and SIP beat the benchmark. |
| Was the fund’s performance consistent? | Ahead in 5 of 6 years<br>59% rolling win rate[^hdfc-rolling] | The fund beat the benchmark in most calendar years and 59% of rolling 3-year periods. |
| Did the extra return justify the extra risk? | Sharpe ratio[^hdfc-sharpe]: 0.77 vs 0.56<br>Volatility: ~17% — similar to the benchmark | Volatility was similar, but the higher Sharpe ratio indicates better risk-adjusted returns. |
...
| How long did recovery take? | Days to recover: 256 vs 228<br>About 1 year for both | Both recovered in about a year. The rapid COVID rebound is not a forecast for future downturns. |

Next, we examine each question in detail using the [Deepdive](https://deepdive.fundinvestigator.com/) analysis.

## Question 1: Did the fund beat a fair benchmark?

A flexi-cap fund can invest across large-, mid-, and small-cap companies. Under [SEBI’s](https://www.sebi.gov.in/) category rules, the Nifty 500 TRI is the benchmark for this category because it covers the same broad range of companies.
...
![Deepdive KPI row showing HDFC Flexi Cap Fund IRR of 19.9%, CAGR of 19.7% and total return of 214.4%, each ahead of the Nifty 500 TRI benchmark](../../assets/images/reports/five-checks-mutual-fund/hdfc-kpi-returns.png)
...
![Deepdive annual returns chart for HDFC Flexi Cap versus Nifty 500 TRI, 2020 to 2026 year to date, showing the fund behind in 2020 and ahead in every subsequent year](../../assets/images/reports/five-checks-mutual-fund/hdfc-annual-returns.png)

*Figure 2: Calendar-year returns for the HDFC Flexi Cap Fund and the Nifty 500 TRI benchmark.*

![Deepdive rolling 3-year returns chart for HDFC Flexi Cap versus Nifty 500 TRI, with a 59% rolling win rate](../../assets/images/reports/five-checks-mutual-fund/hdfc-rolling-returns.png)
...
![Deepdive KPI tiles showing HDFC Flexi Cap Fund Sharpe ratio of 0.77 and volatility of 17.5%, against the Nifty 500 TRI benchmark](../../assets/images/reports/five-checks-mutual-fund/hdfc-kpi-risk.png)
...
![Deepdive drawdown comparison chart for HDFC Flexi Cap versus Nifty 500 TRI, annotated with the 40% COVID fall and the roughly one-year recovery to the previous peak](../../assets/images/reports/five-checks-mutual-fund/hdfc-drawdown-recovery.png)
...
Use the same framework to examine our [Parag Parikh Flexi Cap](/reports/ppfas-flexicap-five-checks/) and [ICICI Prudential Large Cap](/reports/icici-largecap-five-checks/) investigations.
...
[^hdfc-benchmark]: Nifty 500 TRI is the broad-market reference used for this flexi-cap example. The category rationale is discussed in Question 1.
